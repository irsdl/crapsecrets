...
from crapsecrets.base import check_all_modules, carve_all_modules
...
def test_carve_all_body():
    # Test carving from provided HTML body (without making an HTTP request)
    for sample in [aspnet_viewstate_sample, telerik_dialogparameters_sample, jwt_html]:
        result_list = carve_all_modules(body=sample)
        assert len(result_list) > 0

    # Now simulate HTTP responses with httpx.get using respx.
    for idx, sample in enumerate([aspnet_viewstate_sample, telerik_dialogparameters_sample, jwt_html]):
        url = f"http://{idx}.carve-all.badsecrets.com/"
        respx.get(url).mock(return_value=httpx.Response(200, text=sample))
        response = httpx.get(url)
        result_list = carve_all_modules(requests_response=response)
        assert len(result_list) > 0
...
def test_carve_all_cookies():
    ...
    url = "http://cookies.carve-all.badsecrets.com/"
    respx.get(url).mock(
    ...
    response = httpx.get(url)
    result_list = carve_all_modules(requests_response=response)
    # Adjust the expected count based on what carve_all_modules detects.
...
def test_carve_multiple_vulns():
    ...
    """
    url = "http://multiplevulns.carve-all.badsecrets.com/"
    respx.get(url).mock(return_value=httpx.Response(200, text=multiple_vuln_html))
    response = httpx.get(url)
    result_list = carve_all_modules(requests_response=response)
...
def test_carve_empty_vstate():
    ...
    """
    url = "http://emptyvstate.carve-all.badsecrets.com/"
    respx.get(url).mock(return_value=httpx.Response(200, text=empty_vstate_html))
    response = httpx.get(url)
    result_list = carve_all_modules(requests_response=response)
    assert result_list
    assert result_list[0]["product"] == "EMPTY '__VSTATE' FORM FIELD"
