...
########################################
# Now for the tests that call carve() on various modules.
########################################

def test_carve_aspnet_viewstate():
    x = ASPNET_Viewstate()
    # Direct carve using body string
...
def test_carve_headers():
    # Using httpx instead of requests to get a response with custom headers.
    x = Generic_JWT()
    ...
    url_vuln = "http://vuln.headerscarve.badsecrets.com/"
    respx.get(url_vuln).mock(
    ...
    url_notvuln = "http://notvuln.headerscarve.badsecrets.com/"
    respx.get(url_notvuln).mock(
        return_value=httpx.Response(
            200,
    ...
    res = httpx.get(url_notvuln)
    r = x.carve(requests_response=res)
    print(r)
    assert len(r) > 0
    assert r[0]["type"] == "IdentifyOnly"

def cookies_to_headers(cookies: dict) -> list:
        # Convert cookies dictionary into a list of ("set-cookie", "name=value") tuples.
        return [("set-cookie", f"{name}={value}") for name, value in cookies.items()]
...
def test_carve_cookies():
    ...
    url_ps = "http://ps_token.badsecrets.com/"
    respx.get(url_ps).mock(
        return_value=httpx.Response(
            200,
    ...
    url_django = "http://django.badsecrets.com/"
    respx.get(url_django).mock(
        return_value=httpx.Response(
            200,
            headers=cookies_to_headers(cookies),
            text="<html><p>Some HTML Content</p></html>"
    ...
    url_flask = "http://flask.badsecrets.com/"
    respx.get(url_flask).mock(
        return_value=httpx.Response(
            200,
            headers=cookies_to_headers(cookies),
            text="<html><p>Some HTML Content</p></html>"
    ...
    url_rails = "http://rails.badsecrets.com/"
    respx.get(url_rails).mock(
        return_value=httpx.Response(
            200,
            headers=cookies_to_headers(cookies),
            text="<html><p>Some HTML Content</p></html>"
    ...
    # generic_jwt (HMAC and RSA examples)
    x = Generic_JWT()
    cookies = {
        "random-cookie": "useless_data",
    ...
    url_generic1 = "http://hmac.generic-jwt.badsecrets.com/"
    respx.get(url_generic1).mock(
        return_value=httpx.Response(
            200,
            headers=cookies_to_headers(cookies),
            text="<html><p>Some HTML Content</p></html>"
    ...
    url_generic2 = "http://rsa.generic-jwt.badsecrets.com/"
    respx.get(url_generic2).mock(
        return_value=httpx.Response(
            200,
            headers=cookies_to_headers(cookies),
            text="<html><p>Some HTML Content</p></html>"
...
def test_multiple_results():
    # rails_secretkeybase returns two results.
    x = Rails_SecretKeyBase()
    cookies = {
        "random-cookie": "useless_data",
        "rails_session": "eyJfcmFpbHMiOnsibWVzc2FnZSI6IklraGxiR3h2TENCSklHRnRJR0VnYzJsbmJtVmtJSEpoYVd4ek5pQkRiMjlyYVdVaElnPT0iLCJleHAiOm51bGwsInB1ciI6ImNvb2tpZS5zaWduZWQifX0%3D--eb1ea3ddc55deb16ffc58ac165edfbb554067edc",
    ...
    print(r)
    assert len(r) == 2
    expected = "4698bc5d99f3103ca76ab57f28a6b8f75f5f0768aab4f2e3f3743383594ad91f43e78c1b86138602f5859a811927698180ebfae7c490333f37b87521ca5a5f8c"
    assert r[0]["secret"] == expected
    assert r[1]["secret"] == expected
...
def test_generic_jwt_body_carve():
    ...
    """
    url = "http://body.generic-jwt.badsecrets.com/"
    with respx.mock:
        respx.get(url).mock(return_value=httpx.Response(200, text=jwt_html))
        res = httpx.get(url)
        x = Generic_JWT()
        r = x.carve(requests_response=res)
        assert r
...
def test_carve_negativeidentify_body():
    ...
    <html>
    """
    url = "http://negativeidentify.jsf_viewstate.badsecrets.com/"
    with respx.mock:
        respx.get(url).mock(return_value=httpx.Response(200, text=identify_html))
        res = httpx.get(url)
        r = x.carve(requests_response=res)
        assert r
        assert r[0]["type"] == "IdentifyOnly"
...
def test_carve_negative():
    ...
    """
    url = "http://negative.generic-jwt.badsecrets.com/"
    with respx.mock:
        respx.get(url).mock(return_value=httpx.Response(200, text=useless_html))
        res = httpx.get(url)
        r = x.carve(requests_response=res)
        assert not r
    ...
    """
    url = "http://identifyonly.generic-jwt.badsecrets.com/"
    with respx.mock:
        respx.get(url).mock(return_value=httpx.Response(200, text=useless_html))
        res = httpx.get(url)
        r = x.carve(requests_response=res)
...
def test_invalid_carve_args():
    ...
    x = Generic_JWT()
    url = "http://invalidcarveargs.generic-jwt.badsecrets.com/"
    with respx.mock:
        respx.get(url).mock(
            return_value=httpx.Response(200, text=useless_html, headers=cookies_to_headers(cookies))
        )
        res = httpx.get(url)

    with pytest.raises(crapsecrets.errors.CarveException):
